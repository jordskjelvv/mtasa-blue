...
{
    m_pBuildingRemovals = new std::multimap<uint32_t, SBuildingRemoval*>;
    m_pDataBuildings = new std::multimap<uint32_t, sDataBuildingRemovalItem*>;
    m_pBinaryBuildings = new std::multimap<uint32_t, sBuildingRemovalItem*>;
}

void CBuildingRemovalSA::RemoveBuilding(uint32_t usModelToRemove, float fRange, float fX, float fY, float fZ, char cInterior, size_t* pOutAmount)
{
    ...
    m_pBuildingRemovals->insert(std::pair<uint32_t, SBuildingRemoval*>(usModelToRemove, pRemoval));

    bool bFound = false;
    uint uiAmount = 0;
    // Init loop variables
    std::pair<std::multimap<uint32_t, sDataBuildingRemovalItem*>::iterator, std::multimap<uint32_t, sDataBuildingRemovalItem*>::iterator>
                                                                             iterators = m_pDataBuildings->equal_range(usModelToRemove);
    std::multimap<uint32_t, sDataBuildingRemovalItem*>::const_iterator iter = iterators.first;
    ...
    std::pair<std::multimap<uint32_t, sBuildingRemovalItem*>::iterator, std::multimap<uint32_t, sBuildingRemovalItem*>::iterator> iteratorsBinary =
        m_pBinaryBuildings->equal_range(usModelToRemove);
    std::multimap<uint32_t, sBuildingRemovalItem*>::const_iterator iterBinary = iteratorsBinary.first;
...
bool CBuildingRemovalSA::RestoreBuilding(uint32_t usModelToRestore, float fRange, float fX, float fY, float fZ, char cInterior, uint* pOutAmount)
{
    bool bSuccess = false;
    uint uiAmount = 0;

    // Init some variables
    std::pair<std::multimap<uint32_t, SBuildingRemoval*>::iterator, std::multimap<uint32_t, SBuildingRemoval*>::iterator> iterators =
        m_pBuildingRemovals->equal_range(usModelToRestore);
    std::multimap<uint32_t, SBuildingRemoval*>::const_iterator iter = iterators.first;
    ...
    std::multimap<uint32_t, sDataBuildingRemovalItem*>::const_iterator iterator = dataBuildingIterators.first;
    ...
    std::pair<std::multimap<uint32_t, sBuildingRemovalItem*>::iterator, std::multimap<uint32_t, sBuildingRemovalItem*>::iterator>
                                                                         binaryBuildingIterators = m_pBinaryBuildings->equal_range(usModelToRestore);
    std::multimap<uint32_t, sBuildingRemovalItem*>::const_iterator iteratorBinary = binaryBuildingIterators.first;
...
bool CBuildingRemovalSA::IsModelRemoved(uint32_t usModelID)
{
    return m_pBuildingRemovals->count(usModelID) > 0;
}

// Check if a given model is replaced
bool CBuildingRemovalSA::IsDataModelRemoved(uint32_t usModelID)
{
    return m_pBuildingRemovals->count(usModelID) > 0;
}
...
void CBuildingRemovalSA::ClearRemovedBuildingLists(uint* pOutAmount)
{
    ...
    uint                                                             uiAmount = 0;
    std::multimap<uint32_t, SBuildingRemoval*>::const_iterator iter = m_pBuildingRemovals->begin();
    ...
    // Init some variables
    std::multimap<uint32_t, sDataBuildingRemovalItem*>::const_iterator iterator = m_pDataBuildings->begin();
    ...
    // Init some variables
    std::multimap<uint32_t, sBuildingRemovalItem*>::const_iterator iteratorBinary = m_pBinaryBuildings->begin();
    // Loop through the data building list
...
void CBuildingRemovalSA::AddDataBuilding(CEntitySAInterface* pInterface)
{
    if (m_pAddedEntities.find((DWORD)pInterface) == m_pAddedEntities.end() || m_pAddedEntities[(DWORD)pInterface] == false)
    {
        ...
        m_pDataBuildings->insert(std::pair<uint32_t, sDataBuildingRemovalItem*>((uint32_t)pInterface->m_nModelIndex, pBuildingRemoval));
...
void CBuildingRemovalSA::AddBinaryBuilding(CEntitySAInterface* pInterface)
{
    if (m_pAddedEntities.find((DWORD)pInterface) == m_pAddedEntities.end() || m_pAddedEntities[(DWORD)pInterface] == false)
    {
        ...
        // Insert it with the model index so we can fast lookup
        m_pBinaryBuildings->insert(std::pair<uint32_t, sBuildingRemovalItem*>((uint32_t)pInterface->m_nModelIndex, pBuildingRemoval));
        m_pAddedEntities[(DWORD)pInterface] = true;
        m_pRemovedEntities[(DWORD)pInterface] = false;
    }
}
...
void CBuildingRemovalSA::RemoveWorldBuildingFromLists(CEntitySAInterface* pInterface)
{
    ...
    }
    {
        ...
        std::multimap<uint32_t, sDataBuildingRemovalItem*>::const_iterator iterator = dataIterators.first;
    ...
    }
    {
        ...
        std::multimap<uint32_t, sBuildingRemovalItem*>::const_iterator iteratorBinary = binaryIterators.first;
