...
{
public:
    float GetStatValue(std::uint32_t usIndex);
    void  ModifyStat(std::uint32_t usIndex, float fAmmount);
    void  SetStatValue(std::uint32_t usIndex, float fAmmount);

    std::uint32_t GetSkillStatIndex(eWeaponType type);
};
